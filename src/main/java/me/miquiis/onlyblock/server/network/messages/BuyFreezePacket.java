...
public class BuyFreezePacket implements IPlayerSelectablePacket {

    private PlayerSelectorScreen.PlayerSelectorInfo playerSelectorInfo;

    public BuyFreezePacket()
    {

    }

    public BuyFreezePacket(PlayerSelectorScreen.PlayerSelectorInfo playerSelectorInfo)
    {
        this.playerSelectorInfo = playerSelectorInfo;
    }

    public static void encode(BuyFreezePacket msg, PacketBuffer buf) {
        buf.writeCompoundTag(msg.playerSelectorInfo.serializeNBT());
    }

    public static BuyFreezePacket decode(PacketBuffer buf) {
        return new BuyFreezePacket(PlayerSelectorScreen.PlayerSelectorInfo.deserializeNBT(buf.readCompoundTag()));
    }

    public static void handle(final BuyFreezePacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            onlyMoneyBlock.setFrozen(onlyMoneyBlock.getDays(), true);
        });
