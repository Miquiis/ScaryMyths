...
import me.miquiis.onlyblock.server.network.messages.BuySabotagePacket;
...
public class ShopScreen extends Screen {
    ...
    public ShopScreen(String title, List<ItemSlot> itemsToSell) {
        super(new StringTextComponent(title));
        this.title = title;
        this.isBuyActive = true;
        buyStore = itemsToSell;
    ...
    protected void init() {
        ...
        this.purchaseButton = this.addButton(new Button(0, 0, 0, 0, new StringTextComponent("purchase"), p_onPress_1_ -> {
            OnlyBlockNetwork.CHANNEL.sendToServer(new BuyItemFromShopPacket(selectedItem.itemStack, selectedItem.value));
        }));
        ...
        JHTML.Canvas(1920, 1080, true,
                JHTML.Box(1920,1080,true, new Color(0,0,0,150).getRGB(),
                        JHTML.Canvas(1920,220,true,
                                JHTML.Text(0, 0, 160, 0, 0, "$" + (getCurrency() != null ? getCurrency().getCash() : "0"), 5f, true, new Color(46, 217, 91).getRGB()).setCenteredHorizontally()
                        ),
                        JHTML.Box(1920,100,true, new Color(0,0,0,150).getRGB(),
                            JHTML.Text(0, 0, 5, 0, 0, title, 5f, false, Color.WHITE.getRGB()).setCenteredHorizontally().setCenteredVertically()
                        ),
                        JHTML.Canvas(980,70,false,
                                JHTML.Canvas(490, 65, true,
                                        JHTML.Text(0, 0, 15, 0, 0, "Buy Items", 4f, true, Color.WHITE.getRGB()).setCenteredHorizontally().setOnRenderEvent((x, y, w, h) -> {
    ...
    {
        JHTML.Box box = JHTML.Box(195, 5, 0, 2, 0, 0, Color.WHITE.getRGB());
        return isBuy ? box.setCenteredHorizontally() : box;
    }
    ...
    private IOnlyMoneyBlock getCurrency()
    {
        return OnlyMoneyBlock.getCapability(Minecraft.getInstance().player);
    }
