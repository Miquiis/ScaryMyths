...
@OnlyIn(Dist.CLIENT)
public class LeaderboardInventoryScreen extends DisplayEffectsScreen<PlayerContainer> implements IRecipeShownListener {
    ...
    private final LeaderboardGui leaderboardGui = new LeaderboardGui();
    private boolean removeRecipeBookGui;
    private boolean widthTooNarrow;
    private boolean buttonClicked;

    public LeaderboardInventoryScreen(PlayerEntity player) {
    ...
    protected void init() {
        if (this.minecraft.playerController.isInCreativeMode()) {
            ...
            this.leaderboardGui.init(this.width, this.height, this.minecraft, this.widthTooNarrow, recipeBookGui.isVisible());
            this.removeRecipeBookGui = true;
            this.guiLeft = this.recipeBookGui.updateScreenPosition(this.widthTooNarrow, this.width, this.xSize);
            if (leaderboardGui.isVisible())
                this.guiLeft = this.leaderboardGui.updateScreenPosition(this.widthTooNarrow, this.width, this.xSize);
            this.children.add(this.recipeBookGui);
            this.children.add(this.leaderboardGui);
            ...
            this.addButton(new ImageButton(this.guiLeft + 104, this.height / 2 - 22, 20, 18, 0, 0, 19, RECIPE_BUTTON_TEXTURE, (button) -> {
                ...
                this.leaderboardGui.toggleVisibility();
                this.guiLeft = this.recipeBookGui.updateScreenPosition(this.widthTooNarrow, this.width, this.xSize);
                if (leaderboardGui.isVisible())
                    this.guiLeft = this.leaderboardGui.updateScreenPosition(this.widthTooNarrow, this.width, this.xSize);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.recipeBookGui.isVisible() && this.widthTooNarrow) {
            ...
            if (this.leaderboardGui.isVisible() && this.widthTooNarrow) {
                this.drawGuiContainerBackgroundLayer(matrixStack, partialTicks, mouseX, mouseY);
                this.leaderboardGui.render(matrixStack, mouseX, mouseY, partialTicks);
            } else {
                this.leaderboardGui.render(matrixStack, mouseX, mouseY, partialTicks);
