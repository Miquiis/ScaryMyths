...
public class EnergyExpSword extends SwordItem {
    public EnergyExpSword(IItemTier tier, int attackDamageIn, float attackSpeedIn, Properties builderIn) {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        if (!worldIn.isRemote)
        {
            ...
            for (double i = -wideAngle/2; i <= wideAngle/2; i+= gap)
            {
                ...
                DamageableExperienceOrbEntity orbDamage = new DamageableExperienceOrbEntity(worldIn, vec.getX(),vec.getY(),vec.getZ(),10);
